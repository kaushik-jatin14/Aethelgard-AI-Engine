...
function App() {
  ...
  useEffect(() => {
    ...
    if (userProfile.isGuest) { setAppState('CHAR_SELECT'); return; }
    const save = localStorage.getItem(`AETHELGARD_SAVE_${userProfile.pin}`);
    if (save) {
      try {
        const { gameState: gs, chatHistory: ch, character: c } = JSON.parse(save);
        setGameState(gs || INITIAL_GAME_STATE);
        setChatHistory(ch || []);
        setCharacter(c || null);
        setAppState(c ? 'PLAYING' : 'CHAR_SELECT');
      } catch { setAppState('CHAR_SELECT'); }
    } else { setAppState('CHAR_SELECT'); }
  }, [userProfile]);

  useEffect(() => {
    if (appState === 'PLAYING' && character && userProfile && !userProfile.isGuest) {
      localStorage.setItem(`AETHELGARD_SAVE_${userProfile.pin}`, JSON.stringify({ gameState, chatHistory, character }));
    }
  ...
  const handleSelectCharacter = async (char) => {
    ...
    try {
      ...
      setChatHistory([{ role: 'gm', text: `**The Oracle is silent.** ${e.message}\n\nOpen Settings → Ancient Secrets to set thy Gemini Oracle Key.` }]);
  ...
  const handlePlayerAction = async (action) => {
    ...
    try {
      ...
      setChatHistory([...updated, { role: 'gm', text: `**The Oracle falters:** ${e.message}` }]);
    } finally { setIsProcessing(false); }
  };

  const handleReset = () => {
    setAppState('LOGIN'); setUserProfile(null); setCharacter(null);
    setGameState(INITIAL_GAME_STATE); setChatHistory([]); setIsMapOpen(false);
  };

  // Current location data for atmosphere
  const locData = locations.find(l => l.name === gameState.location) || locations[0];
  ...
  const renderScreen = () => {
    if (appState === 'LOGIN') return (
      <>
        ...
        <SettingsPanel isOpen={showSettings} onClose={() => setShowSettings(false)}
          volume={volume} setVolume={setVolume} muted={muted} setMuted={setMuted}
          brightness={brightness} setBrightness={setBrightness} onOpenHelp={() => setShowHelp(true)}/>
        <HelpModal isOpen={showHelp} onClose={() => setShowHelp(false)} onLogin={(profile) => {
          handleLogin(profile);
          setShowHelp(false);
        }} />
      </>
    ...
    if (appState === 'CHAR_SELECT') return (
      <>
    ...
    return (
      ...
      <div className={`relative z-50 flex items-center justify-between px-6 py-4 transition-all duration-700 ${cinematicMode ? '-translate-y-full absolute top-0 w-full opacity-0' : 'translate-y-0 opacity-100 bg-[var(--bg-dark)] border-b border-[var(--border-stone)] shadow-xl'}`}>
        ...
        {/* Center — Majestic Location Header */}
        ...
        {/* Right — Actions (Widely separated) */}
      ...
      <div className="absolute top-28 left-6 z-[60] flex flex-col gap-3">
        {/* The Rune of Sight (ESC) */}
        <button {...withSounds({ onClick: () => setCinematicMode(!cinematicMode) })}
        ...
        {/* The Ancient Tome (Sidebar Toggle) */}
        <button {...withSounds({ onClick: () => setTomeOpen(!tomeOpen) })}
      ...
      <div className="absolute top-4 right-4 z-[60] flex gap-3">
        <button {...withSounds({ onClick: () => setIsMapOpen(true) })}
        ...
        <button {...withSounds({ onClick: () => setShowSettings(true) })}
          className="p-3 rounded-full shadow-2xl transition-all hover:scale-110 backdrop-blur-md"
      ...
      {/* ── THE ANCIENT TOME (Sliding Left Drawer) ── */}
      <AnimatePresence>
        {tomeOpen && (
          <motion.div initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} transition={{ type: 'spring', damping: 25, stiffness: 200 }}
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-6">
              ...
              <div>
                ...
                {(gameState.quests || []).length === 0 ? (
                  ...
                  (gameState.quests || []).map((q, i) => (
                    <div key={q.id || i} className="mb-3 p-4 rounded-md shadow-inner" style={{ background: 'var(--bg-card)', border: '1px solid var(--border-stone)' }}>
                      <p className="text-sm font-ancient font-bold uppercase tracking-wider" style={{ color: 'var(--text-parchment)' }}>{q.title}</p>
                      <p className="text-sm italic mt-2 leading-relaxed" style={{ color: 'var(--text-dim)', fontFamily: 'Crimson Text, serif' }}>{q.description}</p>
                      <span className="mt-3 inline-block badge-ancient text-xs">{q.status}</span>
                    </div>
                  ))
                )}
              </div>
                ...
                {(gameState.inventory || []).length === 0 ? (
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {(gameState.inventory || []).map((item, i) => (
                      <div key={i} className="text-sm p-2 flex items-center gap-2 rounded bg-black/40 border border-stone-800">
              ...
              <div>
                ...
                {(locData?.threats || []).map((t,i) => (
                  <p key={i} className="text-sm mb-1.5 flex items-center gap-2" style={{ color: 'var(--iron-red)', fontFamily: 'Crimson Text, serif' }}>
                    <Swords size={12}/> {t}
                  </p>
                ))}
              </div>
